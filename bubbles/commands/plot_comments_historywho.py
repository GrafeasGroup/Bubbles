...
def plot_comments_historywho_command(message_data: Dict) -> None:
    ...
    response = client.conversations_history(
        channel=rooms_list["new_volunteers"], latest=lastDatetime, limit=number_posts
    )  # ID for #bottest
    ...
    for message in response["messages"]:
        ...
        if "reactions" not in message.keys():
            ...
            countReactionsPeople["Nobody"][differenceDays] = (
                countReactionsPeople["Nobody"][differenceDays] + 1
            )
        else:
            ...
            for reaction in message["reactions"]:
                ...
                if reaction["name"] not in ["heavy_check_mark", "watch"]:
                    ...
                    if (
                        ...
                        if (
                            differenceDays
                            not in countReactionsPeople["Conflict"].keys()
                        ):
                            countReactionsPeople["Conflict"][differenceDays] = 0
                        noValableReaction = False
                        countReactionsPeople["Conflict"][differenceDays] = (
                            countReactionsPeople["Conflict"][differenceDays] + 1
                        )
                        ...
                        if nameUserWhoHasReacted != namePersonToSearch:
                            ...
                            if (
                                differenceDays
                                not in countReactionsPeople["Other"].keys()
                            ):
                                countReactionsPeople["Other"][differenceDays] = 0
                                countReactionsPeople["Other"][differenceDays] = (
                                    countReactionsPeople["Other"][differenceDays] + 1
                                )
                        else:
                            ...
                            if (
                                differenceDays
                                not in countReactionsPeople[namePersonToSearch].keys()
                            ):
                                countReactionsPeople[namePersonToSearch][
                                    differenceDays
                                ] = 0
                            countReactionsPeople[namePersonToSearch][differenceDays] = (
                                countReactionsPeople[namePersonToSearch][differenceDays]
                                + 1
                            )
                        noValableReaction = False
    ...
    response = client.files_upload(
        channels="#bottest", file="plotHourMods.png", title="Just vibing.", as_user=True
    )
