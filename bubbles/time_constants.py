...
TRIGGER_4_HOURS_AGO = datetime.now().replace(hour=(NOW.hour // 4) * 4) + timedelta(hours=4)
TRIGGER_12_HOURS_AGO = datetime.now().replace(hour=(NOW.hour // 12) * 12) + timedelta(hours=12)
